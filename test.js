...
// Fetch pets based on category name and show spinner while loading
const loadCardByCategory = (categoryName) => {
  ...
  fetch(
    ...
    .then((res) => res.json())
    .then((data) => {
      displayPetsInfo(data.data); // Access the 'data' array
      // hideSpinner(); // Hide spinner after data is loaded
    })
    .catch((error) => {
      console.log(error);
      // hideSpinner(); // Hide spinner even if there is an error
    });
};
// Fetch all pets
const loadPets = () => {
